...
4. Соберите и запустите сервисы:
   ...
   у которых дата прогулки меньше текущей даты, и раз в минуту повторяет неотправленные
   Telegram-уведомления о заявках. Если контейнер был перезапущен после 06:00,
   пропущенная очистка выполняется сразу после запуска.
...
| `TELEGRAM_WEBHOOK_SECRET` | Секрет длиной не менее 32 символов для callback-запросов между scheduler и приложением |
